...
export function clock_res_get(id, resolution_ptr)
{
	...
	switch (id) {
		case CLOCKID_REALTIME:
			view.setBigUint64(resolution_ptr, BigInt(1e6), true);
			break;

		case CLOCKID_MONOTONIC:
		case CLOCKID_PROCESS_CPUTIME_ID:
		case CLOCKID_THREAD_CPUTIME_ID:
			view.setBigUint64(resolution_ptr, BigInt(1e3), true);
			break;
...
export function clock_time_get(id, precision, time_ptr)
{
	const view = new DataView(memory.buffer);
	...
	switch (id) {
		case CLOCKID_REALTIME: {
			const t = Date.now();
			const time = BigInt(t) * BigInt(1e6);
...
export function fd_fdstat_get(fd, stat_ptr)
{
	return ERRNO_NOSYS;
}
...
export function fd_filestat_get(fd, buf_ptr)
{
	return ERRNO_NOSYS;
}
...
export function fd_seek(fd, offset, whence, newoffset_ptr)
{
	return ERRNO_NOSYS;
}
...
export function path_filestat_get(fd, flags, path_ptr, path_len, buf_ptr)
{
	return ERRNO_NOSYS;
}
...
export function path_open(fd, dirflags, path_ptr, path_len, oflags, fs_rights_base, fs_rights_inherting, fdflags, opened_fd_ptr)
{
	return ERRNO_NOSYS;
}

export function path_readlink(fd, path_ptr, path_len, buf_ptr, buf_len, bufused_ptr)
{
	return ERRNO_NOSYS;
}
...
export function poll_oneoff(in_ptr, ptr_ptr, nsubscriptions, nevents_ptr)
{
	return ERRNO_NOSYS;
}
...
export function sock_send(fd, si_data_ptr, si_data_len, si_flags, so_datalen_ptr)
{
	return ERRNO_NOSYS;
}
